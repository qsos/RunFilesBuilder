#!/bin/sh
# 彻底停掉旧的
killall -9 tailscale tailscaled 2>/dev/null
/etc/init.d/tailscaled stop 2>/dev/null
rm -rf /etc/tailscale/tailscaled.state

# 拷贝并强制给最高权限
mkdir -p /bin /www/cgi-bin /usr/lib/lua/luci/controller /usr/lib/lua/luci/view/tailscale_web
cp -f bin/tailscale* /bin/
cp -f www/cgi-bin/tailscale_api /www/cgi-bin/
cp -f usr/lib/lua/luci/controller/*.lua /usr/lib/lua/luci/controller/
cp -f usr/lib/lua/luci/view/tailscale_web/*.htm /usr/lib/lua/luci/view/tailscale_web/

chmod 755 /bin/tailscale*
chmod 755 /www/cgi-bin/tailscale_api
chmod -R 755 /usr/lib/lua/luci/view/tailscale_web

# 写入启动脚本
cat << 'EOF' > /etc/init.d/tailscaled
#!/bin/sh /etc/rc.common
START=99
USE_PROCD=1
start_service() {
  procd_open_instance
  procd_set_param command /bin/tailscaled --port 41641 --state /etc/tailscale/tailscaled.state
  procd_set_param respawn
  procd_close_instance
}
EOF
chmod 755 /etc/init.d/tailscaled
/etc/init.d/tailscaled enable
/etc/init.d/tailscaled start
rm -rf /tmp/luci-*
